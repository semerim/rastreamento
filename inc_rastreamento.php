...
define("MINUTOS_OFFSET_HORARIO_TERMOMETRO", parametro("MINUTOS_OFFSET_HORARIO_TERMOMETRO"));
...
// 	}
	...
	// busca todas as tabelas contendo info da classe "listEvent sro" - cada tabela é um evento

	$className="listEvent sro";
	$nodesTables = $xpath->query("//*[contains(@class, '$className')]");
	
	if ($nodesTables->length == 0) {
		// echo 'ERRO 3';
		if (strpos($resposta, $className) == false)
...
*/ 
	...
	// varre todas as tabelas contendo info da classe "listEvent sro" - cada tabela é um evento
	$table = array();
	foreach ($nodesTables as $nodeTable) {
		//get all rows from the table
		$rows = $nodeTable->getElementsByTagName('tr');
...
function rastreioObjParseJSON( $objetos = null, $retornarPaginaInteira = false )
{

	$urlJSON = "https://proxyapp.correios.com.br/v1/sro-rastro/" . $objetos;
...
// in most cases, you should set it to true
    ...
    // $resposta = utf8_decode(curl_exec($ch));
    $resposta = curl_exec($ch);
    // echo "ENCODING: " . mb_detect_encoding(curl_exec($ch));
    curl_close($ch);
    ...
    $arrEventos1 = $obj['objetos'][0];

	if (! is_array($arrEventos1)) {
		return "Pacote não localizado - ERRO 0";
	}

	if (! isset($arrEventos1['eventos'])) {
        return "Pacote não localizado - ERRO 0";
    }

    $arrEventos = $arrEventos1['eventos'];
    ...
    foreach ($arrEventos as $arrEvento) {
    	// dumpVar($arrEvento);
    	$codigo = $arrEvento['codigo'];
    	$descricao = $arrEvento['descricao'];
        // $descricao = $arrEvento['descricao'];
    	$dtHrCriado = str_replace ("T", " ", $arrEvento['dtHrCriado']);

    	$time = strtotime($dtHrCriado);
    	...
    	if (isset($arrEvento['unidade']['endereco']['cidade'])) {
            $cidade = $arrEvento['unidade']['endereco']['cidade'];
            $uf = $arrEvento['unidade']['endereco']['uf'];
            $local = $cidade . " - " . $uf;
            $tipoUnidade = $arrEvento['unidade']['tipo'];
            $descricao = $descricao . " (" . $tipoUnidade . ")";
		}

        if (isset($arrEvento['unidade']['nome'])) {
            $pais = $arrEvento['unidade']['nome'];
            $local = $pais;
            $tipoUnidade = $arrEvento['unidade']['tipo'];
            $descricao = $descricao . " (" . $tipoUnidade . ")";
        }

        if (isset($arrEvento['unidadeDestino'])) {
    		$destino = "";

    		$cidadeDestino = "";
    		if (isset($arrEvento['unidadeDestino']['endereco']['cidade'])) {
                $cidadeDestino = $arrEvento['unidadeDestino']['endereco']['cidade'];
                $destino = $cidadeDestino . " - ";
            }

            $ufDestino = "";
    		if (isset($arrEvento['unidadeDestino']['endereco']['uf'])) {
                $ufDestino = $arrEvento['unidadeDestino']['endereco']['uf'];
                $destino = $destino . $ufDestino;

            }
            $descricao = $descricao . " => " . $destino;

            $tipoUnidadeDestino = $arrEvento['unidadeDestino']['tipo'];
            $descricao = $descricao . "  (" . $tipoUnidadeDestino . ")";
...
// $conexao = getConexao ();
$query = "SELECT cod_rastreamento, nome, url_rastreamento, url1, url1_desc, url2, url2_desc, url3, url3_desc, url4, url4_desc, url5, url5_desc, altura_iframe, DATE_FORMAT(dt_envio, '%d-%m-%Y') dt_envio, seq FROM objeto WHERE status = 1 ORDER BY $ordem";
...
if ($nroRegistros > 0) {
	...
	foreach ($arr1 as $row) {
		if ($indice == "number") {
			...
			$arrPacotes[] = array ($nome, $cod_rastreamento, $altura_iframe, $url1_desc, $url1, $url2_desc, $url2, $url3_desc, $url3, $url4_desc, $url4, $url5_desc, $url5, $seq);
		}
		else {
			...
			$arrPacotes[] = array ('nome' => $nome, 'cod_rastreamento' => $cod_rastreamento, 'altura_iframe' => $altura_iframe, 'url1_desc' => $url1_desc, 'url1' => $url1, 'url2_desc' => $url2_desc, 'url2' => $url2, 'url3_desc' => $url3_desc, 'url3' => $url3, 'url4_desc' => $url4_desc, 'url4' => $url4, 'url5_desc' => $url5_desc, 'url5' => $url5, 'seq' => $seq);
		}
...
$arrEventos = rastreioObjParseJSON($rastreio);
// print_r($arrEventos);
...
foreach ($arrEventos as $evento) {
	...
	if ($nroEvento == 0) {
		...
		$msgTelegram = utf8_encode("<pre>" . $evento[0] . ($evento[1] !== '' ? " - " . $evento[1] : "") . "</pre>\n\n" . $strEvento);
		
		// se o pacote não foi localizado zera a data da última atualização
		// if ($evento[0] == "Pacote não localizado") {
		// if (preg_match('#^Pacote não localizado#i', $evento[0]) === 1) {
		if (comecaCom ($evento[0], 'Pacote não localizado')) {
	...
	$strEvento = destacaEventoRastreamento($evento[1], $strEvento);
...
function comecaCom ($stringao, $comeco) {
	
if (preg_match('#^' . $comeco . '#i', $stringao) === 1)
	return true;
else
	return false;
...
if ($edicao != "1") {
	...
	if ($tipo == "date") {
		...
		$retorno .= "onclick='return showCalendar(" . chr (34) . $campo . chr (34) . ", " . chr (34) . "%d/%m/%Y" . chr (34) . ");' alt=''>";
	}

	if ($tipo == "datetime") {
		// coloca o calendário
		$retorno .= "<img style='vertical-align: middle;' src='img/date.gif' width=20 height=18 ";
		$retorno .= "onclick='return showCalendar(" . chr (34) . $campo . chr (34) . ", " . chr (34) . "%d/%m/%Y %H:%M:%S" . chr (34) . ", " . chr(34) . "24" . chr (34) . ", " . chr (34) . "true" . chr (34) . ");' alt=''>";
	}
...
// dumpVar('PrimeiraChave: ' . $primeiraChave);
// dumpVar('tabelaquery: ' . $tabelaQuery);
...
// dumpVar('Primeira Chave: ' . $primeiraChave);
// dumpVar('TabelaPrinc: ' . $tabelaPrinc);
// dumpVar('Tipo: ' . $tipo);
...
if ($tipo === "TEXT") {
	$primeiraParteID = $tabelaPrinc . "." . $campoFiltro;
    $nomeFiltro = $campoFiltro;
    if ($primeiraChave == "") {
		$primeiraChave = "''";
		$valorPrimeiraChave = "";
...
if ($campoFiltro != $campoChave) {
	$id = "concat(" . $primeiraParteID . ", convert (" . $tabelaPrinc . "." . $campoChave . ", char))";
}
else {
	$id = $tabelaPrinc . "." . $nomeFiltro;
...
function destacaEventoRastreamento ($local, $evento) {
	...
	$arrTrocas = array (array ("aiu para entrega", "", "<FONT COLOR='#FF0000'><B>", "</B></FONT>", "1"),
						array ("Objeto entregue ao destinatário", "", "<FONT COLOR='1C16EF'><B>", "</B></FONT>", "1"),
						array ("CAMPOS DOS GOYTACAZES", "", "<FONT COLOR=#FC7070><B>", "</B></FONT>", "1"),
						array ("Campos Dos Goytacazes", "", "<FONT COLOR=#FC7070><B>", "</B></FONT>", "1"),
						array ("SAO GONCALO", "", "<FONT COLOR=#5F9F9F><B>", "</B></FONT>", "1"),
						array ("Aguardando pagamento", "", "<FONT COLOR=#F28E00><B>", "</B></FONT>", "1"),
						...
						array ("entrega cancelada", "", "<FONT COLOR='#CC6500'><B>", "</B></FONT>", "1"),
						array ("Aguardando pagamento do despacho postal", "Aguardando pagamento do despacho postal", "<FONT COLOR='E66914'><B>", "</B></FONT>", "0"),
						array ("bjeto pago", "", "<FONT COLOR='#278E10'><B>", "</B></FONT>", "1"),
						array ("Pagamento confirmado", "", "<FONT COLOR='278E10'><B>", "</B></FONT>", "1")

						);

// 	array ("Aguardando pagamento do despacho postal", "Aguardando pagamento do despacho postal", "<FONT COLOR='E66914'><B>" . imgHTML('money', '1', 3), imgHTML('money', '1', 3) . "</B></FONT>", "0"),
//	array ("Objeto entregue ao destinatário", "", "<FONT COLOR='1C16EF'><B>" . imgHTML('thumbsup', '1', 3), imgHTML('thumbsup', '1', 3) . "</B></FONT>", "0"),
//	array ("AAAAAAAObjeto saiu para entrega ao destinatário", "", "<FONT COLOR='#FE0000'><B>" . imgHTML('entrega', '1', 3), imgHTML('entrega', '1', 3) . "</B></FONT>", "0"),
//	array ("aiu para entrega", "", "<FONT COLOR='#FE0000'><B>" . imgHTML('entrega', '1', 3), imgHTML('entrega', '1', 3) . "</B></FONT>", "1")

	foreach ($arrTrocas as $troca) {
		$textoOriginal = strtoupper($troca[0]);
		$textoNovo = $troca[1];
		$preTexto = $troca[2];
		$posTexto = $troca[3];
...
/*
			dumpVar('Evento: ' . $evento);
            dumpVar('Local: ' . $local);
			dumpVar('Troca: ' . $textoOriginal);
			dumpVar(strpos($retorno, $textoOriginal));
			dumpVar(strpos($local, $textoOriginal));
			echo "<br>";
*/

			if ((strpos($evento, $textoOriginal) !== false) or (strpos($local, $textoOriginal) !== false)) {
				// dumpVar('Achou');
...
function execComando($seq_servidor, $comando) {
...
$sql = "SELECT s.*, l.descricao, l.ip_servidores FROM servidor s, localidade l WHERE l.seq = s.cod_localidade AND s.seq = '$seq_servidor'";
// dumpvar($sql);
...
if ($nroRegistros > 0) {
	foreach ($arrAssoc as $arrRet) {
		...
		$redeLocal = "0";
		$timeout = '1';		// timeout padrão para execução do comando
		...
		if ($redeLocal === "0") {
			...
			$timeout = '5';		// aumenta o timeout para rede externa
		}
		...
		$strSshpass = ($password === '' ? '' : ' sshpass -p "' . $password . '"');

		// $comando = 'timeout ' . $timeout . ' sshpass -p "' . $password . '" ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $comando;
		$comando = 'timeout ' . $timeout . $strSshpass . ' ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $comando;
		// dumpVar($comando);
		$ret = shell_exec($comando);
...
function buscaStatServidor($seq_servidor, $todos = "0", $gravar = "1", $notificar = "1", $somente_temperatura = "0", $ligaDesligaMonitor = "0") {
...
$titulo = ($somente_temperatura === "1" ? "TEMPERATURAS" . " - " . $now : ($ligaDesligaMonitor === "1" ? "LIGA/DESLIGA MONITORAMENTO" : "MONITORAMENTO"  . " - " . $now));
$tamTitulo = ((($somente_temperatura === "1") or ($ligaDesligaMonitor === "1")) ? "400" : "800");
...
if ($ligaDesligaMonitor === "1")
	$sql = "SELECT s.*, l.descricao, l.ip_servidores FROM servidor s, localidade l WHERE l.seq = s.cod_localidade ORDER BY ordem";
...
if ($nroRegistros > 0) {
	...
	foreach ($arrAssoc as $arrRet) {
		...
		if ($ligaDesligaMonitor === "1") {
			// gera array para painel liga/desliga
			if ($ativo === "1")
				$tdStyle = "servidor_ativo";
			else
				$tdStyle = "servidor_inativo";
			
			$arrServidores[] = array($nome_servidor, $seq, $tdStyle);
			continue;
		}		
		...
		// dumpVar($arrServidores);
		$redeLocal = "0";
		$timeout = '5';		// timeout padrão para execução do comando
		foreach ($arrServidores as $servidor0) {
			if ($servidor0 === $ipLocal) {
				// está na rede local - usar o IP interno
				$redeLocal = "1";
		...
		if ($redeLocal === "0") {
			...
			$ip = $ip_externo;
			$porta_ip = $porta_ip_externo;
			$timeout = '10';		// aumenta o timeout para rede externa
		}
		...
		$nro_vezes_down = (int)$arrRet["nro_vezes_down"];
		$nro_vezes_down_aviso = (int)$arrRet["nro_vezes_down_aviso"];
		...
		if ($password != "")
			$strSshpass = ' sshpass -p "' . $password . '"';
		else
			$strSshpass = "";

		// $comando = 'timeout ' . $timeout . $strSshpass . ' ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' uptime';
		$comando = 'timeout ' . $timeout . $strSshpass . ' ssh ' . $username . '@' . $ip . ' uptime';
		// dumpVar($comando);
		$ret = shell_exec($comando);
		// dumpVar($ret);
		...
		// dumpVar($ultParte);
		$uptime = str_replace (", ", " ", trim (str_replace("," . $ultParte, " ", $parte3)));
...
*/
		...
		if ($uptime === "") {
			// tratar o nro de vezes que o servidor esteve down
			// dumpVar ('Uptime: ' . $uptime);
			$nro_vezes_down++;
			if ($nro_vezes_down >= $nro_vezes_down_aviso) {
				// atualizar 
				$query1 = 'update servidor set nro_vezes_down = ' . $nro_vezes_down . ' where seq = ' . $seq;
				simpleSelectPDO($query1);

				// atingiu o limite - notificar
				if (($down_notificar === "1") and ($notificar === "1")) {
					dumpVar("Comando: " . $comando);
					dumpVar("RET: " . $ret);
					dumpVar("IP: " . $ip);
					dumpVar("Porta: " . $porta_ip);
				...
				// não notificar, somente atualiza o nro de vezes
				$query1 = 'update servidor set nro_vezes_down = ' . $nro_vezes_down . ' where seq = ' . $seq;
				simpleSelectPDO($query1);
			}
			...
			// zerar o nro de vezes que o servidor esteve down
			$query1 = 'update servidor set nro_vezes_down = 0 where seq = ' . $seq;
			simpleSelectPDO($query1);
		}
		...
		if ($temperatura == "1") {
			...
			if ($temperatura_script != "") {
				// $comando = 'timeout ' . $timeout . ' sshpass -p "' . $password . '" ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $temperatura_script;
				$comando = 'timeout ' . $timeout . $strSshpass . ' ssh ' . $username . '@' . $ip . ' ' . $temperatura_script;
				// dumpVar($comando);
				$ret = trim (shell_exec($comando));
				// dumpVar($ret);

                $temp_atual = "";

                // temperatura Home Assistant
                // formato :      551142 (dividir por 10000)
                $temp_ha = '';
                if (is_numeric ($ret)) {
					// dumpVar($ret);
					if (((float)$ret) > 100)
                  		$temp_ha = (float)$ret / 1000;
					else
						$temp_ha = (float)$ret;
                    // $temp_ha = substr ($ret, 0, 2);
                    $temp_atual = $temp_ha;
                    // dumpVar("ha = " . $temp_ha);
                }
                else {
                    ...
                    // temperatura Linux
                    // formato :      temp=53.7'C
                    $temp_linux = trim (pedaco (pedaco ($ret, "=", 2), "'", 1));

                    // temperatura UDM Pro
                    // formato :      temp1_input: 51.500
                    $temp_udmpro = trim (pedaco ($ret, ":", 2));
                    // dumpVar("udmpro = " . $temp_udmpro);

                    // temperatura Switch Unifi
                    // formato :      MAC-1 (Local)    59           Normal           63            0
                    // $temp_swunifi = trim (pedaco (pedaco ($ret, ")", 2), "N", 1));
                    $temp_swunifi = trim (pedaco ($ret, ":", 2));
                    // dumpVar("swunifi = " . $temp_swunifi);

                    // temperatura Linux
                    // formato :      temp=53.7'C
                    $temp_linux = trim (pedaco (pedaco ($ret, "=", 2), "'", 1));
                    // dumpVar("linux = " . $temp_linux);

    //                if (is_numeric ($temp_ha))
    //                    $temp_atual = $temp_ha;

                    if (is_numeric ($temp_udmpro))
                        $temp_atual = $temp_udmpro;

                    if (is_numeric ($temp_swunifi))
                        $temp_atual = $temp_swunifi;

                    if (is_numeric ($temp_linux))
                        $temp_atual = $temp_linux;

                }

                // dumpVar ($temp_atual);
                $temp_atual = round ($temp_atual, 1);
                // dumpVar ($temp_atual);
                $temp_atual = number_format((float)$temp_atual, 1, '.', '');
                // dumpVar ($temp_atual);
			...
			if ($temperatura_url != "") {
				...
				$temp_timestamp_ult_leitura = date('Y-m-d H:i:s', strtotime(MINUTOS_OFFSET_HORARIO_TERMOMETRO . ' minutes', strtotime($temp_timestamp_ult_leitura)));
		...
		if (($cpu == "1") and ($somente_temperatura != "1") and ($ligaDesligaMonitor != "1")) {
			$comando = 'timeout ' . $timeout . $strSshpass . ' ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $cpu_script;
			// dumpVar($comando);
			$ret = shell_exec($comando);
		...
		if (($memoria == "1") and ($somente_temperatura != "1") and ($ligaDesligaMonitor != "1"))  {
			$comando = 'timeout ' . $timeout . $strSshpass . ' ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $memoria_script;
			// dumpVar($comando);
			$ret = shell_exec($comando);
			$mem_str = $ret;
			...
			$mem_load = (float)$mem_str;
			$mem_free = 100 - $mem_load;

			$titulo = "";
			$legendas = "Usado^Livre";
			// $valores = "$mem_used_real^$mem_free";
			$valores = "$mem_load^$mem_free";

			// $retDetalhe = msgBanner ("Memória<br><br>Usado: $mem_used_real MB &nbsp;&nbsp;&nbsp; Livre: $mem_free MB", $class_header, "center", "380");
			$retDetalhe = msgBanner ("Memória<br><br>Usado: $mem_load % &nbsp;&nbsp;&nbsp; Livre: $mem_free %", $class_header, "center", "380");
			...
			$mem_used_real = ($mem_load == "") ? "0" : $mem_load;
		...
		if (($espaco_disco == "1") and ($somente_temperatura != "1") and ($ligaDesligaMonitor != "1"))  {
			$arrDiscos = explode (",", $espaco_disco_script);
			$discoAtual = "1";
			foreach ($arrDiscos as $espaco_disco_script0) {
				$comando = 'timeout ' . $timeout . $strSshpass . ' ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $espaco_disco_script0;
				// dumpVar($comando);
				$ret = shell_exec($comando);
				// dumpVar($ret);
				...
				$linha_disco = trim (preg_replace('!\s+!', ' ', $ret));
				// dumpVar($linha_disco);
				// $disco_size = str_replace ('G', '', trim (pedaco ($linha_disco, " ", 2)));
				$disco_size = round((float)(pedaco ($linha_disco, " ", 2) / 1048576));
				// $disco_used = str_replace ('G', '', trim (pedaco ($linha_disco, " ", 3)));
				$disco_used = round((float)(pedaco ($linha_disco, " ", 3) / 1048576));
				// $disco_free = str_replace ('G', '', trim (pedaco ($linha_disco, " ", 4)));
				$disco_free = round((float)(pedaco ($linha_disco, " ", 4) / 1048576));
				...
				*/
				...
				$retDetalhe = msgBanner ("Partição: $disco_nome<br><br>Tam: $disco_size GB&nbsp;&nbsp;&nbsp; Usado: $disco_used GB&nbsp;&nbsp;&nbsp; Livre: $disco_free GB ($disco_perc_free%)<br>", $class_header, "center", "380");
	...
	if ($ligaDesligaMonitor == "1") {
		// monta saída somente com os botões de liga/desliga monitoramento dos servidores
		$retGeral = msgBanner ($titulo, " class='sel_header_top_pad' ", "center", $tamTitulo);
		// tabela vertical
		...
		foreach ($arrServidores as $servidor) {
			
			$nomeServidor = $servidor[0];
			$seq = $servidor[1];
			$tdStyle = $servidor[2];
			...
			if ($tdStyle === "servidor_ativo")
				$status = "0";
			else
				$status = "1";
			
			$link = "atualizaStatusServidor.php?seq=" . $seq . "&status=" . $status;
			
			$retTab2 .= "<tr>\n<td width=400 align=center class='" . $tdStyle . "' onClick='self.location=" . chr(34) . $link . chr (34) . "'>\n";

			// $linhaRet = "<a class='" . $tdStyle . "' href='" . $link . "'>" . $nomeServidor . "</a>";
			$linhaRet = $nomeServidor;
...
send_Telegram("\n<b>" . utf8_decode($subject) . "</b>\n\n" . utf8_decode($msgTelegram), $tokenTelegram);
