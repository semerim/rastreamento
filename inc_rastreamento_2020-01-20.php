...
define("LOGIN_TIMEOUT", parametro("LOGIN_TIMEOUT"));	// tempo em minutos para for�ar o login independente de atividade
define("ACTIVITY_TIMEOUT", parametro("ACTIVITY_TIMEOUT"));	// tempo em minutos para for�ar o login baseado na �ltima atividade
...
*/
	...
	if ($resposta == "") {
		// echo 'ERRO 0';
		return 'Pacote n�o localizado - ERRO 0';
	}
	...
	if (is_array($resposta)) {
		// echo 'ERRO 1';
		return 'Pacote n�o localizado - ERRO 1';
	}
	...
	if ($resposta === '') {
		// echo 'ERRO 2';
		return 'Pacote n�o localizado - ERRO 2';
	}
...
// 	}
	...
	if ($tables->length == 0) {
		...
		if (strpos($resposta, "O nosso sistema") != false)
			return "Pacote n�o localizado";
		else
			return "Pacote n�o localizado - ERRO 3";
	}
	...
	foreach ($rows as $row)
{
		...
		foreach ($cols as $node) {
			...
			if (strpos($nodo, "COM ERRO") != false)
				return "P�gina de consulta dos Correios com ERRO";
...
foreach ($arrEventos as $evento) {
	...
	if ($nroEvento == 0) {
		...
		// se o pacote n�o foi localizado zera a data da �ltima atualiza��o
		// if ($evento[0] == "Pacote n�o localizado") {
		// if (preg_match('#^Pacote n�o localizado#i', $evento[0]) === 1) {
		if (comecaCom ($evento[0], 'Pacote n�o localizado')) {
		...
		if (isDate($dataHora, 'd/m/Y H:i:s')) {
			...
			if (isDate($ultDataGravada, 'd/m/Y H:i:s')) {
				...
				if ($dt1 < $dt2) {
					// atualiza status - notifica se necess�rio
...
if ($ipLocal === "") {
	// n�o conseguiu localizar pelo OS, busca o par�metro
...
function destacaEventoRastreamento ($evento) {
	...
	$arrTrocas = array (array ("CAMPOS DOS GOYTACAZES", "", "<FONT COLOR=#FC7070><B>", "</B></FONT>", "1"),
						...
						array ("entrega n�o efetuada", "", "<FONT COLOR='#CC6500'><B>", "</B></FONT>", "1"),
						array ("entrega n�o pode ser efetuada", "", "<FONT COLOR='#CC6500'><B>", "</B></FONT>", "1"),
						...
						array ("Objeto entregue ao destinat�rio", "", "<FONT COLOR='1C16EF'><B>" . imgHTML('thumbsup', '1', 3), imgHTML('thumbsup', '1', 3) . "</B></FONT>", "0"),
						array ("AAAAAAAObjeto saiu para entrega ao destinat�rio", "", "<FONT COLOR='#FE0000'><B>" . imgHTML('entrega', '1', 3), imgHTML('entrega', '1', 3) . "</B></FONT>", "0"),
...
if ($nroRegistros > 0) {
	foreach ($arrAssoc as $arrRet) {
		...
		$redeLocal = "0";
		$timeout = '1';		// timeout padr�o para execu��o do comando
		foreach ($arrServidores as $servidor0) {
			if ($servidor0 === $ipLocal) {
				// est� na rede local - usar o IP interno
				$redeLocal = "1";
			}
		}
		if ($redeLocal === "0") {
			// n�o encontrou na lista de servidores locais - � internet
			$ip = $ip_externo;
...
if ($nroRegistros > 0) {
	...
	foreach ($arrAssoc as $arrRet) {
		...
		// dumpVar($arrServidores);
		$redeLocal = "0";
		$timeout = '5';		// timeout padr�o para execu��o do comando
		foreach ($arrServidores as $servidor0) {
			if ($servidor0 === $ipLocal) {
				// est� na rede local - usar o IP interno
				$redeLocal = "1";
		...
		$comando = 'timeout ' . $timeout . ' sshpass -p "' . $password . '" ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' uptime';
		// dumpVar($comando);
		$ret = shell_exec($comando);
...
*/
		...
		if ($uptime === "") {
			...
			if ($nro_vezes_down >= $nro_vezes_down_aviso) {
				...
				// n�o notificar, somente atualiza o nro de vezes
				$query1 = 'update servidor set nro_vezes_down = ' . $nro_vezes_down . ' where seq = ' . $seq;
				simpleSelectPDO($query1);
			}
		...
		if ($temperatura == "1") {
			...
			if ($temperatura_url != "") {
				...
				if ((($ultimoIntervalo > $temperatura_leitura_timeout) or ($ultimoIntervalo == -1)) and ($lastRead != 0) and ($temperatura_notificar == "1") and ($notificar == "1") ){
					...
					// provavelmente o term�metro n�o est� acessando a internet - notificar
			...
			$retDetalhe = "<center><h2>Temperatura: " . $temp_atual . "�C</h2>" . ($temp_ult_leitura == "" ? "" : "<h4>Lido " . ($temp_ult_leitura == "0" ? "agora" : "h� $temp_ult_leitura minuto(s)") . ".</h4>") . "</center>";
		...
		if (($cpu == "1") and ($somente_temperatura != "1") and ($ligaDesligaMonitor != "1")) {
			$comando = 'timeout ' . $timeout . ' sshpass -p "' . $password . '" ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $cpu_script;
			// dumpVar($comando);
			$ret = shell_exec($comando);
		...
		if (($memoria == "1") and ($somente_temperatura != "1") and ($ligaDesligaMonitor != "1"))  {
			$comando = 'timeout ' . $timeout . ' sshpass -p "' . $password . '" ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $memoria_script;
			// dumpVar($comando);
			$ret = shell_exec($comando);
			...
			if ($mem_used_perc >= ((float)$memoria_limite)) {
				if ($memoria_notificar == "1") {
					$assuntoNotificar = "ALERTA - Mem�ria - " . $nome_servidor . " (" . $ip . ") [" . $ipLocal . "]";
		...
		if (($espaco_disco == "1") and ($somente_temperatura != "1") and ($ligaDesligaMonitor != "1"))  {
			...
			foreach ($arrDiscos as $espaco_disco_script0) {
				$comando = 'timeout ' . $timeout . ' sshpass -p "' . $password . '" ssh -p ' . $porta_ip . ' -o StrictHostKeyChecking=no ' . $username . '@' . $ip . ' ' . $espaco_disco_script0;
				// dumpVar($comando);
				$ret = shell_exec($comando);
				// dumpVar($ret);
		...
		foreach ($arrMostrados as $celula) {
			...
			if ($celula[2] != "") {
				// assunto da notifica��o preenchido - notificar por email e Telegram
				$sendTo = parametro("EMAIL_NOTIFICACAO_MONITORAMENTO");
				$subject = $celula[2];
				$body = $celula[3];
				// dumpVar($body);
	...
	if ($somente_temperatura == "1") {
		// monta sa�da somente com as temperaturas, ignora as outras m�tricas
		$retGeral = msgBanner ($titulo, " class='sel_header_top_pad' ", "center", $tamTitulo);
		// tabela vertical
	...
	if ($ligaDesligaMonitor == "1") {
		// monta sa�da somente com os bot�es de liga/desliga monitoramento dos servidores
		$retGeral = msgBanner ($titulo, " class='sel_header_top_pad' ", "center", $tamTitulo);
		// tabela vertical
